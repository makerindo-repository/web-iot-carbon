    ...
    <x-slot name="header">
        <h2 class="leading-tight">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item breadcrumb-active">{{ __('Tambah Threshold') }}</li>
            </ol>
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="sm:max-w-7x xl:max-w-full mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg px-4">
                <div class="p-6">
                    <h1 class="text-3xl font-extrabold mb-4">Tambah Data Threshold</h1>
                    <form action="{{ route('rsc-data.sensor-threshold.store') }}" method="POST">
                        @csrf
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div class="w-full">
                                <x-input-label for="parameter">{{ __('Parameter') }}</x-input-label>
                                <select id="parameter" class="block mt-1 w-full rounded-xl" name="parameter">
                            ...
                            <div class="w-full">
                                ...
                                <x-text-input id="min" class="block mt-1 w-full rounded-xl" type="number"
                                    name="min" :value="old('min')" required autofocus autocomplete="min"
                                    step="0.1" min="0" />
                            ...
                            <div class="w-full">
                                ...
                                <x-text-input id="max" class="block mt-1 w-full rounded-xl" type="number"
                                    name="max" :value="old('max')" required autofocus autocomplete="max" step="0.1" min="0" />
